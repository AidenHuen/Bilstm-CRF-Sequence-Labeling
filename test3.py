...
def get_precision():
    ...
    for i in range(rows_answer.__len__()):
        try:
            ...
            if answer_items.__len__() != 1 and result_items.__len__() != 1:
                if answer_items[answer_items.__len__()-1] != "W" and answer_items[answer_items.__len__()-1] != "S-nre":
                    if "nrf" == answer_items[answer_items.__len__()-1][2:]:
                        recall_num += 1
                    # recall_num += 1
                if result_items[result_items.__len__()-1] != "W" and result_items[result_items.__len__()-1] != "S-nre":
                    if "nrf" == result_items[result_items.__len__()-1][2:]:
                        precision_num += 1
                    # precision_num += 1
                    if answer_items[answer_items.__len__()-1] == result_items[result_items.__len__()-1]:
                        if "nrf" == result_items[result_items.__len__() - 1][2:]:
                            print answer_items[0], result_items[result_items.__len__()-1], answer_items[answer_items.__len__()-1]
                            correct_num += 1
                        # correct_num += 1
                    else:
                        print answer_items[0], result_items[result_items.__len__()-1], answer_items[answer_items.__len__()-1]
                else:
                    if answer_items[1] not in ["B", "S", "W", "O"]:
                        print answer_items[0], result_items[result_items.__len__()-1], answer_items[answer_items.__len__()-1]
        except:
            continue
    ...
    # print precision_num
    precision = correct_num*1.0/precision_num
    recall = correct_num*1.0/recall_num
    print "nrf"
    print "precision:", correct_num*1.0/precision_num

    print "recall:", correct_num*1.0/recall_num
...
def get_precision():
    ...
    for items in items_result:
        ...
        try:
            if items[1][0] == "B" and items[1][2:]=="nrf":
                precision_num += 1
        except:
            pass
            ...
            if items_result[i][1][0] == "B" and items_result[i][1][0] == items_answer[i][1][0]:
                ...
                while j<items_answer.__len__():
                    ...
                    if items_answer[j][1][0] != "E":
                        ...
                        break
                if items_result[j][1][0] == "E" and items_answer[j][1][2:]=="nrf":
